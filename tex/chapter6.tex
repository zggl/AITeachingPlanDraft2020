...
\begin{align}
  ...
  \end{array}\right.
\end{align}
分别给出$CF(H,E)>0,\, CF(H,E)=0,\, CF(H,E)>0$的解释.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
&=\frac{P(H | E)-P(H)}{1-P(H)}=MB(H, E). \hfill \textup{信任增长度}
\end{align}
...
\begin{itemize}
\item 合取: 当组合证据是多个单一证据的组合时, 即 $E=E_1\,\,  AND\,\,    E_2\,\,    AND \,\,   \cdots\,\,     AND \,\,   E_n$时, 若已知$CF(E_1), CF(E_2), \cdots , CF(E_n)$, 则
\begin{align}
...
\begin{align*}
  &CF_1(H)=\textup{CF}(H, E_1) \times\max\{0, \textup{CF}(E_1)\};\\
  &CF_2(H)=\textup{CF}(H, E_2)\times\max\{0, \textup{CF}(E_2)\}.
\end{align*}
...
\begin{align}
 ...
 \begin{array}{llr}
  ...
  {C F_{1}(H)+C F_{2}(H)+C H_{1}(H) \times CF_{2}(H)}, & \textup{若} CF_{1}(H)<0 \\
   {C F_{1}(H)+C F_{2}(H)},                            & \textup{若} \textup{CF}(H)< 0\\
...
由$r_4$得到:
\begin{align*}
    \textup{CF}(E_1)&=0.8\times \max\{ 0, \textup{CF}(E_4  AND  (E_5  OR   E_6)) \}\\
          &=0.8\times \max\{ 0, \min\{\textup{CF}(E_4),  \textup{CF}(E_5  OR   E_6)\} \}\\
...
由$r_1$得到:
\begin{align*}
    CF_1(H)&=\textup{CF}(H, E_1)\times \max\{0,  \textup{CF}(E_1)\}=0.9\times \max\{0,  0.4\} = 0.36.
\end{align*}
    由$r_2$得到:
\begin{align*}
    CF_2(H)&=\textup{CF}(H, E_2)\times \max\{0,  \textup{CF}(E_2)\}=0.6\times \max\{0,  0.8\} = 0.48.
\end{align*}
由$r_3$得到:
\begin{align*}
    CF_3(H)&=\textup{CF}(H, E_3)\times \max\{0,  \textup{CF}(E_3)\}=-0.5\times \max\{0,  0.6\} = -0.3.
\end{align*}
...
\end{aligned}
\end{align}
下面进一步讨论LS和LN的含义. 由本章第 \ref{AI32C6Sec6.2} 节的 Bayes 公式可知:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{证据理论}
证据理论是由德普斯特 (A. P. Dempster) 首先提出, 并由沙佛 (G. Shafer) 进一步发展起来的用于处理不确定性的一种理论, 也称DS (Dempster-Shafer) 理论.
...
\begin{align}\label{AIC6exam6.17}
...
\qquad\ding{174}  全集$\Omega$的概率分配数按(3)计算.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{example}
...
\begin{align}
    \begin{array}{ll}
        &m\left(\left\{h_{1}\right\},\left\{h_{2}\right\}, \ldots,\left\{h_{3}\right\} \right)=\left( \textup{CER}(E) \times c_{1},\, \textup{CER}(E) \times c_{2},\, \cdots,\, \textup{CER}(E) \times c_{n}\right).\\
