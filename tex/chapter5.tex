...
\end{align}
这是公式（18）的正规化结果. 符号$A\circ B$表示矩阵$\bm A$和$\bm B$(Schur积)逐元素相乘. 算法2的步骤4中使用它, 因为它比标准矩阵乘积快.
...
全局来说, 该算法使用$\bm X$的SVD来避免计算问题,并在SVD计算伪逆时引入Tikhonov正则化参数. 
...
\centering
  ...
  \caption{The architecture of the TROP-ELM.}
