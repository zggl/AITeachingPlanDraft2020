...
\begin{align*}
  \begin{array}{ll}
        &\delta_{k}=-\left (d_{k}-y_{k}\right) f^{\prime}\left (I_{k}\right) \\
        &\Delta w_{j k}=\eta\left (d_{k}-y_{k}\right) f^{\prime}\left (I_{k}\right) O_{j}.
  \end{array}
\end{align*}

\ding{171} 如果节点$k$不是输出层上的节点, 则它是隐含层上的节点的, 此时:
\begin{align*}
...
AdaBoost衍生出许多算法, 有一个庞大的算法家族, 统称 Boosting, 是机器学习中 ``集成学习"的主流代表性技术. 即便在当今这个 ``深度学习时代", Boosting仍发挥着重要作用. \index{集成学习}\index{AdaBoost}
\end{example}
%%%%---------------------------------
\begin{example}
