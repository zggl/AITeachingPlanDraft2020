...
DNN是一种用了很多浅层神经网络技巧之外的多层网络, 它能够 ``深"到解决浅层神经网络很难解决的问题.
...
如果我们想更改$x$的维度为$B$的维度的话, 一种方法是采用填充零和下采样的方法, 或者是引入一个维度为$A*B$的变换矩阵, 使每次都乘上这个矩阵.
...
原始底层的映射记作 $H(x)$, 堆叠非线性的映射记作 $F(x) = H(x) - x$, 原始的映射变为$F(x) + x$,
...
图 \ref{DRL2020020301} 就是其构造深度残差网络的构思来源图, 比较使用了56层的网络和一个是20层的网络, 从原理上来说其实56层网络的解空间是包括了20层网络的解空间的, 换而言之也就是说, 56层网络取得的性能应该大于等于20层网络的性能的.
...
在构造这个网络的时候, 如果20层的网络可以取得非常好的结果, 在构造56层网络的时候前20层从20层网络中拷贝过来, 后面的36层只做恒等映射, 至少效果不会差于20层的网络.
深度残差的网络提出的这个思想其实不复杂, 这种网络结构打破了每一层网络输入只能来自于上一层网络输出的规律, 可以让一些网络的输出直接跳过几层到达后面的输入.
这样的网络确实也取得了非常好的效果.
%%%%------------------------------------------
\begin{remark}
在真正训练的时候, 有几点小技巧要注意:
...
将图像级别的分类扩展到像素级别的分类 (图 \ref{FCN20170811112941027}), 获得 CVPR2015 的 最佳论文奖 \cite{Long2015-9593}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{example}
图 \ref{TUCon2020020302} 是卷积运算需要的输入矩阵和卷积核, 图 \ref{JuanJi2020020301} 左上角是计算 第一行第一列像素的卷积后像素值, 其中
%\begin{figure}[H]
%\centering
...
&\cellcolor{inteins}{\footnotesize \quad 7}&\cellcolor{inteins}{\;\;\footnotesize 8}&\cellcolor{inteins}{\footnotesize \;\;9}\\
\end{tabularx}
\end{align*}
\caption{第一行第一列像素值}
\label{TUCon2020020302}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%% M 代码  %%%%%%%%%%%%%%%%%%%%%%%%%%%
\lstinputlisting[language=matlab, caption={卷积运算}]{Code/C7exam727.m}
%%%%%%%%%%%%%%%%%%%%%%%%%%% M 代码  %%%%%%%%%%%%%%%%%%%%%%%%%%%
...
反卷积跟1维信号处理的反卷积计算不一样, FCN称为后向卷积层\index{后向卷积层}, 有人称反卷积层, 也该称为转移卷积层.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{example}
大小为$2\times 2$图像的反卷积过程如图 \ref{20170715175257862}, 其中输入$2\times 2$, 卷积核$4\times 4$, 滑动步长3, 输出$7\times 7$, 图 \ref{20170715175257862}给出了输入为$2\times 2$的图片经过$4\times 4$的卷积核进行步长为3的反卷积的运算过程.
...
%\end{figure}
现在我们继续, 将蓝色框移动两个步长, (1,3)位置卷积的结果为83.
%\begin{align*}
%\begin{array}{|l|l|l|l|l|l|l|}
%\hline 2 & 3 & 7 & 4 & 6 & 2 & 9 \\
%\hline 6 & 6 & 9 & 8 & 7 & 4 & 3 \\
...
%\end{figure}
以此类推, 继续以步长2移动, 会得到91, 127, 最后一行分别是44, 72, 74.
%\begin{figure}[H]
%    \centering
...
输入和输出的维度是由下面的公式决定的.
如果你用一个$f\times f$的过滤器卷积一个$n\times n$的图像, padding为$p$, 步幅为$s$, $3\times 3$的输出变为
\begin{align*}
    \left\lfloor\frac{n+2p-f}{s}+1\right\rfloor \quad \times \quad\left\lfloor\frac{n+2p-f}{s}+1\right\rfloor\Rightarrow \left\lfloor\frac{7+0-3}2+1\right\rfloor\times\left\lfloor\frac{7+0-3}2+1\right\rfloor=3\times 3.
\end{align*}
\end{example}
\begin{remark}
步幅stride是图像处理中常用的概念.
...
按照机器学习的惯例, 通常不进行翻转操作. 从技术上说, 这个操作可能叫做互相关更好. 但在大部分的深度学习文献中都把它叫做卷积运算. 很多机器学习的文献把这种运算叫做卷积运算, 不用翻转操作.
...
描述一个二维矩阵，使用行和列. 三维的，使用通道、行和列. 四维则多了一个参数batch. 四个维度batch channel row col的逻辑顺序则和数据格式有关，常见的有 \href{https://mp.weixin.qq.com/s/I4Q1Bv7yecqYXUra49o7tw?}{NHWC和NCHW}：

假设过滤器窗口是 $3\times 3\times 3$（其中一个3代表了in\_depth）. 有四个这样的窗口,
用于提取同一个图片的四个属性(out\_depth指定，深度对应输出Out channel 0…3). 那么，针对图片里面的某个Batch（譬如Batch 0），其处理流程如下：
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[H]
\centering
...
\ding{171} \href{https://blog.csdn.net/tianwenbo6666/article/details/104349425}{池化（Pooling）}它实际上是一种形式的采样方法.
...
%\end{figure}
池化结果如式 \ref{AIC7Pooling747}.
\begin{align}\label{AIC7Pooling747}
\begin{tabularx}{0.3\textwidth}{llllllllllll}
