...
\begin{center}
    \begin{tikzpicture}[decision/.style={diamond, draw, fill=blue!20, text width=3.5cm, aspect=5,align=flush center, inner sep=0pt},font={\sf \small},scale=0.78]
        ...
        \draw[->] (IsKNsetEmpty)node[left,yshift=-0.7cm] {$Y$}--(CollapSolve);
        \draw[->] (CollapSolve)--(IsNewKN);
        \draw[->] (IsNewKN)node[left,yshift=-0.7cm]{$Y$}--(AddtoKB);
        \draw[->] (IsNewKN)-|(leftabove1.center)-|(IsKNsetEmpty.north);
        ...
        \draw[->] (IsKNsetEmpty)node[left,xshift=3.5cm,yshift=-0.3cm]{$Y$}--(AddNewFact);
...
这种不可满足性可用如下定理描述:
%%%%%%%-----------------------------------------------
\begin{mythm}{子句集的不可满足}{}\label{AIthm3.3}
...
%%%%%%%----------------------------------------
\begin{result}
取$L_1= P(a),  L_2=\neg P(y)$, 则$L_1$和$L_2$的最一般合一是$\sigma={a/y}$. 根据定义\ref{AI32Def321},可得
\begin{align}
C_{12}=( {C_1\sigma }-{L_1\sigma }) \cup  ({C_2 \sigma }-{L_2\sigma })
              &=({P(a), R(x)}-{P(a)})\cup ({\neg P(a), Q(b)}-{\neg P(a)})\notag\\
              &=({R(x)})\cup({Q(b)})= {R(x), Q(b)}\notag\\
...
%%%%%%%----------------------------------------%
\begin{remark}
    对谓词逻辑, 定理\ref{AIthm3.2}仍然适用, 即归结式$C_{12}$是其亲本子句$C_1$和$C_2$的逻辑结论. 用归结式取代它在子句集$S$中的亲本子句, 所得到的子句集仍然保持着原子句集$S$的不可满足性.
此外, 对谓词逻辑定理\ref{AIthm3.3}也仍然适用, 即从不可满足的意义上说, 一阶谓词逻辑的归结原理完备.
\end{remark}
%%%%%%%--------------------------------------
...
\node (Bn)[draw, process,align=center,below=1.5cm of AB0,yshift=0cm,xshift=1.5cm] {$\neg \mathbf{B}(\mathbf{n})$};        %30
...
\node (Ra1)[draw, process,color=blue,align=center,below=1cm of RyLy0,yshift=0cm,xshift=1.2cm] {$\textcolor[rgb]{0.02,0.0,1}{\neg \mathbf{Q}(\mathbf{w})}$};
\draw (RyLy0)--(Ra1);
\draw (La0)-- (Ra1);
...
%%%%%%%-------------------------------------------------
\begin{example}
对上述目标公式的与/或形, 可用如下的与/或树\ref{AI32fig2019120130}表示.
\begin{figure}[H]
\begin{center}
\begin{tikzpicture}[font={\sf \small}]
\def \smbwd{2cm}
